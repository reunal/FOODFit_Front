...
import "../../styles/periodCheck/PeriodCheck.css";
...
const PeriodCheck = () => {
    ...
    const fetchTypeData = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/api/analysis/period?startDate=${moment(startDate).format(
            "YYYY-MM-DD"
          )}&endDate=${moment(endDate).format("YYYY-MM-DD")}&nutrient=${selectedType}`,
          {
            headers: {
  ...
  const period = (periodData) => {
    ...
    for (let i = 0; i < t; i++) {
      ...
      setDataList((prevDataList) => [...prevDataList, periodData.amountList.slice(start, end)]);
    }
  };
  ...
  const formatDate = (value) => {
    let periodData = new Date(value);
    let modify_date =
      (periodData.getMonth() + 1 < 9
        ? "0" + (periodData.getMonth() + 1)
        : periodData.getMonth() + 1) +
      "/" +
      (periodData.getDate() < 9 ? "0" + periodData.getDate() : periodData.getDate());
    console.log(periodData);
    return modify_date;
  };
  ...
  return (
    <div className="period_page">
      ...
      <div className="select_div">
        <select className="select_type" onChange={handleSelectType} value={selectedType}>
          ...
          {dataList.map((item, index) => (
            <div className="graph_data">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart width={150} height={40} border_radius={"10px"} data={item}>
                  <ReferenceLine
